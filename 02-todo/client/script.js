...
function renderTask({ id, title, description, dueDate ,completed}) {
  ...
  */
  ...
  När eventlyssnaren kopplas till knappen här nedanför, görs det däremot i HTML-kod och inte JavaScript. Man sätter ett HTML-attribut och refererar till eventlyssnarfunktionen istället. Då fungerar det annorlunda och parenteser är tillåtna. */
    const taskStation = completed == true ? "checked" : "";
    const taskEnd = completed == true ? "opacity-25" : "";
    const taskText = completed ? "text-blue-500 ":" "; 
  let html = `
    <li class="select-none mt-2 py-4 border-b border-amber-300 ${taskEnd}">
      <div class="flex items-center">
      <input type="checkbox" value="${id}" onclick="updateTask(${id})" ${taskStation}/>
        <h3 class="mb-3 flex-1 text-xl font-bold text-pink-800 uppercase ${taskText} "> ${title}</h3>
        <div>
          <span>${dueDate}</span>
          <button onclick="deleteTask(${id})" class="inline-block bg-red-500 text-xs text-black-200 border border-white px-5 py-2 rounded-md ml-2">Ta bort</button>
        </div>
      </div>`;
